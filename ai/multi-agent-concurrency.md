...
- **Titles got the same treatment.** The identical-title problem was not only a branch bug: it left the sidebar full of indistinguishable rows, and the approval dialog and completion notification both identify a chat *by title*, so "which chat wants to run this command?" had no answer. New chats are titled `owner/repo (agent <id>)` reusing the branch's conversation-id slice, so chat, title and worktree share one handle. Applied in the existing PATCH (no extra round trip); the backend marks it `title_custom`, which is right — it is deliberate, not auto-derived.
...
- Still open, pre-existing and deliberately untouched: if worktree provisioning fails and the user declines the "continue on the current branch" fallback, the already-created conversation is left behind unbound — the same orphan the function's own header comment says it avoids for the repo-registration failure path. Also, a repo with no commits yet has no base to cut a worktree from, so it lands in that same fallback.
