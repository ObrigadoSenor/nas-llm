...
## Phase 16 — Multi-agent chats: parallel runs, per-chat branches, completion notifications 🚧
Source plan: `67796345-a780-42f3-bb0b-44611675303e`; detail log: `ai/multi-agent-concurrency.md`. Goal: run several agent chats at once across repos without them killing each other, give each chat a real isolated branch, and notify when a chat finishes unwatched. Root causes found in source first: a single job worker (a browser-relay job *blocks* it while awaiting the browser, so one local chat stalled all others); a single SSE tail whose drop *cancelled* the job (`handleEvents` on `j.local`), so switching chats killed the running chat; `genTimeout` 5m bounding approval-gated tool waits; one git working tree per repo, with `repo_branch` stored but never sent to the sidecar; and `showApprovalDialog` wiring its resolver only on first creation, so only the first approval of a session ever resolved (the same bug already fixed in `dsConfirm`). Built as four parallel child agents (Go backend / Rust sidecar / web UI / renderer) in worktrees, merged conflict-free onto `orchestrator/mc-integrate`.
- [x] Backend (`backend/`): worker pool (`MAX_CONCURRENT_JOBS`, default 4) replacing the single worker, with a per-host semaphore of 1 so server inference stays serialized per `OLLAMA_NUM_PARALLEL=1` while browser-relay jobs (which run on the user's machine) parallelize freely. New user-scoped multiplexed `GET /api/events` (`eventHub`) carrying `modelCall`/`toolExec`/`phase`/`done`/`joberror`, every payload tagged `convId`+`jobId`, terminal payloads carrying `status`/`error`. Instant disconnect-cancel replaced by `scheduleGraceCancel` (`BROWSER_RELAY_GRACE`, 45s) checking both the per-conversation tail and the user's global stream. Split timeouts: `AGENT_JOB_TIMEOUT` 30m, `TOOL_EXEC_TIMEOUT` 15m, plain chat still 5m. `toolExecPayload` gains `convId`+`branch`; `injectRepoContext` now prefers the conversation's branch over the shared tree's.
- [x] Sidecar (`desktop/src-tauri/`): `ensure_worktree` gives each `(repo, branch)` its own checkout under `<data_dir>/worktrees/…`, handling main-tree reuse, existing-worktree reuse, local branch, origin-only branch (tracking checkout) and create-new. `repos_exec` + state/diff/revert/commit/create-pr/changelog/ship take an **optional** `branch` (absent ⇒ today's exact behaviour); new `POST /__sidecar/repos/worktree`. `tauri-plugin-notification` added + registered + `notification:default` capability. Cache-bust bumped (`desktop.js` v20, `desktop.css` v17).
- [x] Web UI (`www/`): `activeLocalAbort` singleton replaced by a per-`convId` map (that shared abort was what let one chat kill another's relay); `closeTail` no longer aborts any relay. Global `/api/events` consumer drives background chats headlessly (optional renderer/bubble in `relayLocalModelCall`), skipping the foreground `convId`, with backoff reconnect and graceful degradation on an older backend. `finishJob` dispatches `nasllm:jobDone` unconditionally (deduped by jobId), plus a stacked toast and a sidebar finished badge for non-active chats.
- [x] Renderer (`desktop/renderer/`): `showApprovalDialog` re-wires its resolver and handlers on every call and queues approvals FIFO, each labelled with the asking chat; `toolExec` shim reads `convId` from the payload and passes `branch` through to the sidecar; per-chat branch chip + picker (switch or create, ensures the worktree, PATCHes `repoBranch`) with the worktree caveat surfaced; native notification on `nasllm:jobDone`, suppressed only when the chat is on screen *and* focused.
- [x] Validated on `orchestrator/mc-integrate`: `gofmt`/`go vet`/`go build`/`go test`/`go test -race` clean; `cargo check` clean; `node --check` clean on all three ES modules. Plus live probes against a throwaway backend (temp DB/port/session, all removed after): **two local chats both `generating` simultaneously**, `/api/events` multiplexing both with `convId`+`jobId`, terminal `status` present, finishing one leaving the other running; grace-period behaviour confirmed in all four states (survives tail drop, survives past grace while the global stream is open, cancelled when everything drops); and a scratch-repo probe confirming the git worktree semantics the isolation relies on.
- [x] Follow-up fix (same branch): `createRepoChat` no longer calls `repos/branch` (which `git switch -c`'d the **shared repo folder**) — it now provisions a worktree via `repos/worktree {create:true}`. Investigating it surfaced a bigger problem: the branch name came from `slugify(title)` and every chat on a repo is titled `owner/repo (agent)`, so **every chat shared one branch** (`agent/obrigadosenor-nas-llm--agent` — the branch this session started on). Branch names are now `agent/<repo-short>-<first 7 of convId>`: unique per chat. Also removes a data hazard — `git switch -c` on a dirty tree carried the user's uncommitted edits onto the chat's branch; a worktree cannot. Proven with a scratch-git probe contrasting old vs new (old: folder branch moved + edit followed; new: folder untouched, edit stays, two chats hold two isolated branches).
- [x] Follow-up (same branch): chat **titles** made unique too — `owner/repo (agent <id>)`, reusing the branch's conversation-id slice so chat, title and worktree share one handle. Applied in the existing PATCH, so no extra request. Identical titles were not only a branch bug: they made the sidebar unreadable and left the new approval dialog and completion notification — both of which name the chat — unable to say *which* chat was asking or finishing.
- [ ] User: `npm run desktop` from `orchestrator/mc-integrate` — run chats on two repos plus two branches of one repo at once, switch between them and reload mid-run (all keep running), confirm each chat's edits land on its own branch, approve two tool calls from different chats in a row, and let an unwatched chat finish with the window unfocused to get a native notification + sidebar badge (macOS prompts for permission the first time).
