...
- `agent_prompt.go` (~170 lines) — all system-prompt builders: `agentSystemNudge` (weak/medium), `agentSystemStrong` (strong tier, lean), `toolCallDiscipline` (weak/medium only), `injectRepoContext` (git, AGENTS.md nudge) / `injectWorkspaceContext` (non-git, AGENTS.md nudge), `injectPlanContext` (Plan mode), `injectMemoryIndex`
