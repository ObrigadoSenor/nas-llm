...
- `workspace_test.go` — non-git workspace (`UseGit=false`) gets file tools but no git tools in the agent allowlist, and uses `injectWorkspaceContext`; `localFileTools`/`localGitTools` partition
